Meteor.methods({
 ...
 addDMNote: function(note){
  ...
  deleteDMnote: function(note){
    var note = DMNotes.findOne({_id: note});
    if (note.dm_id = this.userId) {
      let dm = DMNotes.findOne({_id: note._id});
   return DMNotes.remove({_id: note._id});
  }
  }
